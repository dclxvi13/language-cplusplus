module Language.CPlusPlus.Draft where
...
data Declarator
  ...
  | LambdaDeclarator [Parameter] Bool (Maybe ExceptionSpecification) [AttributeSpecifier] (Maybe TypeId)
...
cppLanguageDef =
  Token.LanguageDef
    ...
    , Token.identStart = nondigit
    , Token.identLetter = nondigit <|> digit
    , Token.opStart = oneOf ""
...
lexer = Token.makeTokenParser cppLanguageDef
